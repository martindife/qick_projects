...
class Axis_QICK_Proc(SocIp):
    ...
    def __init__(self, description):
        ...
        for param in ['lfsr','divider','arith','time_read','qnet','qcom','custom_periph','io_ctrl','ext_flag', 'time_ctrl', 'core_ctrl', 'gen_sync']:
    ...
    def __str__(self):
        lines = []
        lines.append('---------------------------------------------')
        lines.append(' QICK PROCESSOR - TPROC V2 INFO ')
        lines.append('---------------------------------------------')
        ...
        for param in ['has_time_read','has_io_ctrl','has_time_ctrl','has_core_ctrl','has_gen_sync']:
            lines.append("%-14s: %s" % (param, ["NO", "YES"][self.cfg[param]]))
        lines.append("---------- Peripherals ----------")
        for param in ['has_lfsr','has_divider','has_arith']:
            lines.append("%-14s: %s" % (param, ["NO", "YES"][self.cfg[param]]))
        for param in ['has_ext_flag','has_qnet','has_qcom']:
            lines.append("%-14s: %s" % (param, ["NO", "YES"][self.cfg[param]]))
