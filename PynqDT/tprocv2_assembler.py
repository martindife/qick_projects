                ...
                if (label): # add label to label_dict if not already registered.
                    ...
                    if ( directive == 'ALIAS'):
                        ...
                        if (len(directive_params) == 3):
                            ...
                            if (check_name(A_Name)):
                                if A_Name in Alias_List:
                                    error = Logger.error('ALIAS_RECOGNITION', 'Alias Name ' + A_Name  +' already in use in line ' + str(line_number) )
                                else:
                                    if ( check_reg(A_Reg) ):
                                        Alias_List.update({ A_Name : A_Reg } )        
                                        Logger.info("ALIAS_RECOGNITION",' > ' + A_Reg + ' is called ' + A_Name)
                                    else:
                                        error = Logger.error('ALIAS_RECOGNITION', 'Register Name error in line ' + str(line_number) )
                            else:
                                error = Logger.error('ALIAS_RECOGNITION', 'Alias Name Error in line ' + str(line_number) )
                        else:
                            error = Logger.error('ALIAS_RECOGNITION', 'Parameters error in line ' + str(line_number) )
                        ...
                        if (check_lit(C_val)):
                            ...
                            Logger.info("CONSTANT_RECOGNITION",' > ' + C_val + ' is called ' + C_name)
                        else:
                            error = Logger.error('CONSTANT_RECOGNITION', 'CONST '+C_name+' Value '+C_val+' is not a Literal in line ' + str(line_number) )
                    elif ( directive == 'ADDR'):
                        directive_params = list(filter(lambda x:x, command.split(' ')))
                        if  (check_num(directive_params[1])):
                            ...
                            if  (distance < 0):
                                error = Logger.error('ADDR', 'New Memory Address '+str(Value)+ ' before than next empty address ('+str(mem_addr)+') in Line ' + str(line_number))
                            else :                          
                                mem_addr = Value
                        else:
                            error = Logger.error('ADDR', 'Address Value '+Value+ ' error in Line ' + str(line_number))
                    elif ( directive == 'END'):
                        mem_addr += 1            
...
class Instruction():
    ...
    def PORT_WR (current : dict) -> tuple:
        ...
        if (error==0):
            if ('DST' in current):
                rsA1 = integer2bin(current['DST'], 6)
            else:
